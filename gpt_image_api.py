"""Small Python client for GPT Image generation via MuAPI."""
...
class GPTImageAPI:
    """Submit GPT Image generation/edit jobs and poll their results."""

    def __init__(self, api_key=None, base_url="https://api.muapi.ai/api/v1"):
        self.api_key = api_key or os.environ.get("MUAPI_API_KEY")
        if not self.api_key:
            raise ValueError("Set MUAPI_API_KEY or pass api_key to GPTImageAPI")
    ...
    def generate(
        self, prompt, aspect_ratio="auto", resolution="2K", quality="high"
    ):
        """Submit a GPT Image 2 text-to-image job."""
        return self._submit(
    ...
    def edit(
        ...
        """Submit a GPT Image 2 edit with up to sixteen image URLs."""
        if not images_list or len(images_list) > 16:
            raise ValueError("images_list must contain between 1 and 16 image URLs")
        return self._submit(
